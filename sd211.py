...
plt.clf()  # clear any existing figure/plot
for i in range(len(angles)):
...
plt.clf()  # clear any existing figure/plot
for i in range(len(angles)):
    plt.plot(centre_line[i][125:175])
plt.legend(angles)
plt.xlabel('Pixel number')
plt.ylabel('Attenuation value')
plt.savefig('results/week2/varying_with_angles_zoomed_in.png', bbox_inches='tight',pad_inches = 0)
